...
等有了初步的了解，至少能大概写出来，虽然还是有很多东西不知道，比如为什么无法通过id属性找到这个控件并设置它的属性（item标签的title属性不能设置吗？），还有个难点就是android项目中使用了很多设计模式，导致安桌教程的代码有些看不懂。最后github上的图片怎么有时能显示，有时又不能显示呢？一种解决方法是修改hosts文件：C:\Windows\System32\drivers\etc\hosts，添加以下内容。
```
# GitHub Start 
